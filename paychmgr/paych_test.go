...
func TestBestSpendable(t *testing.T) {
	...
	nonce := uint64(1)
	voucherAmount := big.NewInt(1)
	svL1V1 := testCreateVoucher(t, ch, voucherLane, nonce, voucherAmount, fromKeyPrivate)
	_, err := mgr.AddVoucher(ctx, ch, svL1V1, nil, minDelta)
	require.NoError(t, err)

	nonce++
	voucherAmount = big.NewInt(2)
	svL1V2 := testCreateVoucher(t, ch, voucherLane, nonce, voucherAmount, fromKeyPrivate)
	_, err = mgr.AddVoucher(ctx, ch, svL1V2, nil, minDelta)
	require.NoError(t, err)

	nonce++
	voucherAmount = big.NewInt(3)
	svL1V3 := testCreateVoucher(t, ch, voucherLane, nonce, voucherAmount, fromKeyPrivate)
	_, err = mgr.AddVoucher(ctx, ch, svL1V3, nil, minDelta)
	require.NoError(t, err)
	...
	nonce = uint64(1)
	voucherAmount = big.NewInt(2)
	svL2V1 := testCreateVoucher(t, ch, voucherLane, nonce, voucherAmount, fromKeyPrivate)
	_, err = mgr.AddVoucher(ctx, ch, svL2V1, nil, minDelta)
	require.NoError(t, err)
...
func TestCheckSpendable(t *testing.T) {
	...
	nonce := uint64(1)
	voucherAmount := big.NewInt(1)
	voucher := testCreateVoucherWithExtra(t, ch, voucherLane, nonce, voucherAmount, fromKeyPrivate)

	// Add voucher with proof
	minDelta := big.NewInt(0)
	proof := []byte("proof")
	_, err := mgr.AddVoucher(ctx, ch, voucher, proof, minDelta)
	require.NoError(t, err)
...
func TestSubmitVoucher(t *testing.T) {
	...
	_, err := mgr.AddVoucher(ctx, ch, voucher, addVoucherProof, minDelta)
	require.NoError(t, err)
	...
	secret2 := []byte("secret2")
	voucher = testCreateVoucherWithExtra(t, ch, voucherLane, nonce, voucherAmount, fromKeyPrivate)
	_, err = mgr.AddVoucher(ctx, ch, voucher, addVoucherProof2, minDelta)
	require.NoError(t, err)
...
func testCreateVoucherWithExtra(t *testing.T, ch address.Address, voucherLane uint64, nonce uint64, voucherAmount big.Int, key []byte) *paych.SignedVoucher {
	sv := &paych.SignedVoucher{
		ChannelAddr: ch,
		Lane:        voucherLane,
		Nonce:       nonce,
